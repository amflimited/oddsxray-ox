...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/sitemap.xml") {
      return text(res, 404, "No sitemap here. ox.oddsxray.com is the hidden runtime, not the public site.\n");
    }
    ...
    if (url.pathname === "/api/packages") {
      return json(res, 200, {
        ...
        forge: {
          ...
          game_mode: "single_runtime_tracked_evidence"
        },
        deploy: {
          mode: "ox-hyperlift-only",
          manual_commands_required: false,
          note: "This Hyperlift app is only the Ox/runtime surface. The public oddsxray.com website remains a separate deployment."
        },
        seo: {
          ox_noindex: true,
    ...
    if (url.pathname === "/f" || url.pathname === "/forgeup" || url.pathname === "/forge-current.sh" || url.pathname === "/forge-selfpull-install.sh") {
      return text(res, 410, "Deprecated. Forge now runs inside Ox. Do not use curl installers.\n");
    }
...
server.listen(port, "0.0.0.0", () => {
  console.log(`The Ox ${build} running as hidden runtime on port ${port}; local Forge ${localForgeBuild}.`);
});
