...
const sendHtml = (res, status, body) => { res.writeHead(status, { "content-type": "text/html; charset=utf-8" }); res.end(body); };
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/forge-002.sh") return serveRepoFile(res, "forge-002.sh", "text/x-shellscript; charset=utf-8");
  if (url.pathname === "/health") return sendJson(res, 200, { ok: true, app: "Odds X-Ray", layer: "The Ox", build, status: "online", forge_proxy: "/api/forge/health", forge_current: "/forge-current.sh" });
  if (url.pathname === "/api/forge/health") { const out = await proxyForge("/health"); return sendJson(res, out.status, { ok: out.status < 400, ox_build: build, proxied_from: "The Forge", forge: out.body }); }
  ...
  if (url.pathname === "/" || url.pathname.startsWith("/app")) { const page = fs.readFileSync(indexPath, "utf8").replaceAll("OX-004", build).replaceAll("OX-004G", build).replaceAll("OX-003", build); return sendHtml(res, 200, page); }
